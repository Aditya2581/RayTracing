...
def rgb_box():
    p_bottom = Plane.Plane(np.array([0.0, -25.0, 0.0]), np.array([0.0, 1.0, 0.0]))
    p_left = Plane.Plane(np.array([-25.0, 0.0, 0.0]), np.array([-1.0, 0.0, 0.0]))
    p_right = Plane.Plane(np.array([25.0, 0.0, 0.0]), np.array([1.0, 0.0, 0.0]))
    p_top = Plane.Plane(np.array([0.0, 25.0, 0.0]), np.array([0.0, -1.0, 0.0]))
    p_back = Plane.Plane(np.array([0.0, 0.0, 100.0]), np.array([0.0, 0.0, -1.0]))
    ...
    p_top.material(colour=np.array([255.0, 255.0, 255.0]))
    p_back.material(colour=np.array([150.0, 150.0, 150.0]))

    s0 = Sphere.Sphere(0, 0, 50, 5)
    s1 = Sphere.Sphere(15, 15, 70, 5)
    s2 = Sphere.Sphere(-15, 15, 70, 5)
    s3 = Sphere.Sphere(15, -15, 50, 5)
    s4 = Sphere.Sphere(-15, -15, 50, 5)
    s0.material(colour=np.array([200.0, 200.0, 200.0]), smoothness=0.2)
    ...
    s4.material(colour=np.array([163.0, 7.0, 178.0]), smoothness=1.0)

    scene = [p_bottom, p_left, p_back, s0, s1, s2, s3, s4]
    return scene
